...
$result = mysql_query("SELECT id, first_name, last_name, email_address, status, mobile_number, image_link, type
											FROM provider_table
...
if(mysql_num_rows($result) == 1)
{
	...
	if($status > 1)
	{
      ...
      $type = $row[7];
		...
		$token = getToken(32);
      mysql_query("UPDATE provider_table SET token = '$token' WHERE id = '$id' AND status > 1");
      	if(mysql_errno() == 0)
      	{
            $message = "Success";
      	}
         ...
         $responseData = array('message' => $message, 'id' => $id,
                             ...
                             'mobile_number' => $mobile_number ,'image_link' => $image_link,
